	...
	if (isset($return))
	{
		...
		foreach ($keys as $key)
		{
			...
			if (!empty($key))
			{
				if (!$count)
				{
					$alertContent .= "<br>".$key." _est une nouvelle clé_";
				}
				else
				{
					$db->exec("UPDATE ".KEYS_TABLE." SET boxed = 0, date_upd = '{$db->getDatenow()}' WHERE game_key = '$key'");
					$listofkeys .= $key.", ";
					$alertContent .= "<br>".$key." _réintroduite_";
				}
			}
		}
		...
		if (!empty($keys))
		{
			$logs->new('data', $listofkeys."returned to stock");
		}
	}
	else
	{
		...
		{
			$alertTitle = '<i class="fas fa-times"></i>';
			$alertContent = "_Des éléments sont manquants_";
		}
	}
}
...
<form method="POST">
	...
	<div class="form-check">
    	<input type="checkbox" class="form-check-input" id="checkbox1" name="return">
    	<label class="form-check-label" for="checkbox1">_Les clés ci dessous seront réintroduites dans le stock_</label><br>
    	<small>_(les nouvelles clés seront considérées comme des doublons)_</small>
  	</div>
